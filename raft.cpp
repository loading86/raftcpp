...
void Raft::TickHeartbeat()
{
    heart_beat_elapsed_++;
    election_elapsed_++;
    if(election_elapsed_ >= elction_timeout_)
    {
        election_elapsed_ = 0;
        if(check_quorum_)
        {
        ...
        if(state_ == StateLeader && lead_transferee_ != 0)
        {
            AbortLeaderTransfer();
        }
    }
    ...
    if(heart_beat_elapsed_ >= heart_beat_timeout_)
    {
...
void Raft::BecomeLeader()
{
    ...
    int32_t ret = raft_log_->Entries(raft_log_->Commited() + 1, UINT64_MAX, entries);
    if(ret != 0)
    {
        exit(1);
    }
    int32_t conf_num = NumOfPendingConf(entries);
    if(conf_num > 1)
    {
        exit(1);
    }
...
void Raft::RestoreNode(std::vector<uint64_t>& nodes, bool is_learner)
{
    for(auto node: nodes)
    {
        uint64_t match = 0;
        ...
        SetProgress(node, match, next, is_learner);
    }
}
...
bool Raft::Restore(const raftpb::Snapshot& snap)
{
    ...
    for (int i = 0; i < snap.metadata().conf_state().nodes_size(); i++) {
        nodes.push_back(snap.metadata().conf_state().nodes(i));
    }
    RestoreNode(nodes, false);
    nodes.clear();
    for (int i = 0; i < snap.metadata().conf_state().learners_size(); i++) {
        nodes.push_back(snap.metadata().conf_state().learners(i));
    }
    RestoreNode(nodes, true);
    return true;
}
...
void Raft::Campaign(const std::string& compaign_type)
{
    uint64_t term;
    ...
    {
        BecomePreCandidate();
        vote_msg_type = raftpb::MsgPreVote;
        term = term_ + 1;
    }else
    {
        BecomeCandidate();
    ...
    for(auto it = peers_.begin(); it != peers_.end(); ++it)
    {
        ...
        msg.set_to(it->first);
        msg.set_term(term);
        msg.set_type(vote_msg_type);
        msg.set_index(raft_log_->LastIndex());
        uint64_t last_term;
        raft_log_->LastTerm(last_term);
        msg.set_logterm(last_term);
        msg.set_context(compaign_type == kCampaignTransfer? kCampaignTransfer:"");
        Send(msg);
    }
