...
uint64_t MemoryStorage::firstIndex() { return entries_[0].index() + 1; }
...
int32_t MemoryStorage::Entries(uint64_t lo, uint64_t hi, uint64_t maxSize,
...
int32_t MemoryStorage::CreateSnapshot(uint64_t index, raftpb::ConfState cs,
                                      std::string &data, raftpb::Snapshot &ss) {
  uint64_t my_snap_index = snapshot_.metadata().index();
  uint64_t in_snap_index = ss.metadata().index();
  if (in_snap_index <= my_snap_index) {
    return ErrSnapOutOfDate;
...
int32_t MemoryStorage::Append(std::vector<raftpb::Entry> &entries) {
  ...
  if (offset_index = lastIndex() + 1) {
    ...
    uint64_t final_len =
        entries[entries.size() - 1].index() - entries_[0].index() + 1;
